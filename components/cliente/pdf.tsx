...
const PDFView = ({ ...props }) => {
  ...
  return (
    <PDFViewer style={{ height: "100vh", width: "100vw" }}>
      <Document
        title={`Informe de Gestión Tributaria ${props.cliente.nombre_empresa}`}
      >
        <Page style={styles.body}>
          <View style={styles.header} fixed>
            <Image
              src="/images/logo.png"
              style={{ height: 60, width: 80, alignSelf: "flex-start" }}
            />

            <View style={styles.titulo}>
              <Text style={{}}>Informe de Gestión Tributaria</Text>
            </View>
          ...
          <View style={{ ...styles.table, marginTop: 10, borderTop: 1 }}>
            <View style={styles.tableRow}>
              ...
              <Text style={styles.tableCell}>{props.cliente.nit}</Text>
            </View>
          </View>
                ...
                return (
                  <View key={index} wrap={false} style={{ ...styles.tableRow }}>
                    ...
                    <Text style={{ ...styles.tableCell }}>
                      {
                        periods.find(
                          (periodo) => periodo.value == impuesto.frecuencia
                        )?.name
                      }
                    </Text>
                    <Text style={{ ...styles.tableCell }}>
                    ...
                    <Text style={{ ...styles.tableCell }}>
                      {impuesto.fecha_limite?.toLocaleDateString(
                        "es-co",
                        formatoFecha
                      )}
                    </Text>
                    <Text style={{ ...styles.tableCell }}>
                      {impuesto.fecha_presentacion?.toLocaleDateString(
                        "es-co",
                        formatoFecha
                      ) || mensajeFechaNula}
                    </Text>
                    <Text style={{ ...styles.tableCell }}>
                      {impuesto.fecha_pago?.toLocaleDateString(
                        "es-co",
                        formatoFecha
                      ) || mensajeFechaNula}
                    </Text>
          ...
          <View wrap={false} style={{ ...styles.footer }} fixed>
            <View style={{ border: 1, borderColor: "#" }}></View>
            <Text style={{ fontWeight: "bold" }}>
              R&R Consultorias Empresariales S.A.S
            </Text>
            <Text>Correo Electrónico: gerencia@rrconsultorias.com</Text>
            <Text>
              Avenida del Ferrocarril No. 29-200 - Edificio El Mayor Bussiness
              Center Oficina 101 Santa Marta D.T.C.H
            </Text>
          </View>
        </Page>
      </Document>
